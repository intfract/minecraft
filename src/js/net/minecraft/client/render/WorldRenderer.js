...
export default class WorldRenderer {
  ...
  initialize() {
    ...
    this.blockHitBox = new THREE.LineSegments(
      edges,
  ...
  setupFog(x, z, inWater, partialTicks) {
    if (inWater) {
      ...
      voidMesh.material.color.set(new THREE.Color(skyColor.x * 0.2 + 0.04, skyColor.y * 0.2 + 0.04, skyColor.z * 0.6 + 0.1))
  ...
  renderBlockHitBox(player, partialTicks) {
    ...
    if ((this.blockHitBox.visible = hitBoxVisible)) {
      ...
      if (typeId !== 0) {
        ...
        this.blockHitBox.scale.set(width, height, depth)
  ...
  bobbingAnimation(player, stack, partialTicks) {
    ...
    this.translate(stack, Math.sin(walked * 3.141593) * yaw * 0.5, -Math.abs(Math.cos(walked * Math.PI) * yaw), 0.0)
