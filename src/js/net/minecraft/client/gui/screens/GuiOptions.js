...
export default class GuiOptions extends GuiScreen {
  ...
  init() {
    ...
    this.buttonList.push(
      new GuiSwitchButton('Ambient Occlusion', settings.ambientOcclusion, this.width / 2 - 100, y, 200, 20, (value) => {
        settings.ambientOcclusion = value
        this.minecraft.worldRenderer.rebuildAll()
      }),
    )
    this.buttonList.push(
      new GuiSwitchButton('View Bobbing', settings.viewBobbing, this.width / 2 - 100, y + 24, 200, 20, (value) => {
        settings.viewBobbing = value
      }),
    )
    this.buttonList.push(
      new GuiSliderButton('FOV', settings.fov, 50, 100, this.width / 2 - 100, y + 24 * 2, 200, 20, (value) => {
        settings.fov = value
      }),
    )
    this.buttonList.push(
    ...
    this.buttonList.push(
      new GuiButton('Controls...', this.width / 2 - 100, y + 24 * 4, 200, 20, () => {
        this.minecraft.displayScreen(new GuiControls(this))
      }),
    )

    this.buttonList.push(
      new GuiButton('Done', this.width / 2 - 100, y + 130, 200, 20, () => {
