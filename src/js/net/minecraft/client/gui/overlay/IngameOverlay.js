...
export default class IngameOverlay extends Gui {
  ...
  renderHotbar(stack, x, y) {
    ...
    this.drawSprite(
      ...
      24,
      24,
      x + this.minecraft.player.inventory.selectedSlotIndex * 20 - 1,
      y - 1,
      24,
      24,
    )
  ...
  renderLeftDebugOverlay(stack, filters = []) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Clear the line
      if (filters.length !== 0) {
        stack.clearRect(1, 1 + FontRenderer.FONT_HEIGHT * i, this.getStringWidth(stack, lines[i]) + 1, FontRenderer.FONT_HEIGHT)
      }
