...
export default class PlayerListOverlay extends Gui {
  ...
  reinitialize(stack, width) {
    ...
    let columnWidth =
      Math.min(columns * ((isOnlineMode ? 9 : 0) + maxPlayerNameWidth + maxScoreWidth + 13), screenWidth - 50) / columns
    ...
    if (headerLines !== null) {
      ...
      for (let i = 0; i < headerLines.length; i++) {
        this.drawCenteredString(stack, headerLines[i], Math.floor(width / 2) + 1, y)
    ...
    for (let [uuid, playerInfo] of playerInfoMap) {
      ...
      if (i < playerInfoMap.size) {
        ...
        if (isOnlineMode) {
          ...
          this.drawRect(stack, entryX, entryY, entryX + size, entryY + size, 'rgb(0,0,0)')
    ...
    if (footerLines !== null) {
      ...
      for (let i = 0; i < footerLines.length; i++) {
        this.drawCenteredString(stack, footerLines[i], Math.floor(width / 2) + 1, y)
        y += FontRenderer.FONT_HEIGHT
      }
      y++
    }
